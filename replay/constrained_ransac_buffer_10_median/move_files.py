...
def select_indices(latents, distances_map, buffer_size):
    ...
    for indices in tqdm(indices_combos):
        ...
        for i in range(len(latents)):
            ...
            if vid_num not in distances_by_video:
                distances_by_video[vid_num] = []
            distances_by_video[vid_num].append(dist)

        unrepresented_vids = set([v for v in unique_vids if v not in distances_by_video])
        for unrepresented_vid_num in unrepresented_vids:
            ...
            distances_by_video[unrepresented_vid_num] = dist
        
        median_distances = [np.median(distances_by_video[vid_num]) for vid_num in unique_vids]
        score = np.mean(median_distances)
        print(f'Score: {score}')
        if score < min_score:
            min_score = score
